...
runs = [["ISOL_G", "TIDES_G"], ["ISOL8_h", "TIDES8_h_p3_mm"], ["ISOL9_h_p3_mm", "TIDES9_h_p3_mm"]]
sburst = [[0.2, 1.8], [0.2, 1.4], [0.2, 1.4]]
...
# Figure setting
fig, axes = plt.subplots(1, 3, sharey=True, constrained_layout=True, figsize=(11, 4))
...
for i in range(len(sburst)):
    ...
    for j in range(int(len(sburst[i])/2)):  # ONLY WORKS WHEN THERE'S NO MORE THAN 2 STARBURSTS
        axes[i].axvspan(xmin=(sburst[i])[2*j], xmax=(sburst[i])[2*j+1], color=colors[j+1], zorder=zorders_sburst[j], alpha=alphas_squares[j], hatch=hatches[j])
