...
# Parameters
plan = 'xy'  # xy, xz or yz
pic_name = "galaxy_map_" + plan + "_anim_"
...
for index in range(0, len(dumps)):
    ...
    # Animation
    sx1, sy1, sx2, sy2 = separate_stars('unbound_stars_data_v3-3', 'ascii_output/s000', str_dumps[index], sx, sy, ID, plan, choice)
    data_x = list(sx1) + list(sx2)  # sx1 and sy1 are coordinates of either stars that were already formed before the start of the simulation or bound stars
    data_y = list(sy1) + list(sy2)  # sx2 and sy2 are coordinates of either stars formed with the the interaction or intracluster stars
    colors = np.array(['deeppink', 'y'])
    sizes = np.array([0.2, 0.2])
    t = np.array([1.0, 1.0])  # transparency
    cat1, cat2 = [0] * len(sx1), [1] * len(sx2)
