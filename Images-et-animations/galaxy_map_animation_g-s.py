...
#galaxy_name = 'Af_v1'
plan = 'yz'  # xy, xz or yz
pic_name = "galaxy_map_" + plan + "_anim_g-s.mp4"
...
for index in range(0, len(dumps)):
    ...
    # Animation
    data_x = gx.tolist() + sx.tolist()
    data_y = gy.tolist() + sy.tolist()
    colors = np.array(['tab:red', 'y'])
    sizes = np.array([0.2, 0.2])
    g_cat, s_cat = [0]*len(gx), [1]*len(sx)
    categories = np.array(g_cat + s_cat)
