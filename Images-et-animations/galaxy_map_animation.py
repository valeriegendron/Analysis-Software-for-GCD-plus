...
#galaxy_name = 'Af_v1'
plan = 'yz'  # xy, xz or yz
pic_name = "galaxy_map_" + plan + "_anim.mp4"
lim = 500  # max extent of axi in kpc
...
liste_xdata = ['dx', 'gx', 'sx']
liste_ydata = ['dy', 'gy', 'sy']
frames = []
for index in range(0, len(dumps)):
        ...
        if len(fileName) == 4:  # to make sure it is an output file
            ...
            if fileName.startswith("d"):
                if fileName.endswith(str_dumps[index]):
                    dx, dy = np.loadtxt('ascii_output/' + fileName, usecols=columns, unpack=True)
            # Gas
            elif fileName.startswith("g"):
                if fileName.endswith(str_dumps[index]):
                    gx, gy = np.loadtxt('ascii_output/' + fileName, usecols=columns, unpack=True)
            # Stars
            elif fileName.startswith("s"):
                if fileName.endswith(str_dumps[index]):
                    sx, sy = np.loadtxt('ascii_output/' + fileName, usecols=columns, unpack=True)
            else:
                continue
        else:
            continue
    ...
    # Animation
    data_x = dx.tolist() + gx.tolist() + sx.tolist()
    data_y = dy.tolist() + gy.tolist() + sy.tolist()
    colors = np.array(['k', 'tab:red', 'y'])
    sizes = np.array([0.2, 0.8, 0.8])
    d_cat, g_cat, s_cat = [0]*len(dx), [1]*len(gx), [2]*len(sx)
    categories = np.array(d_cat + g_cat + s_cat)
    frame = plt.scatter(data_x, data_y, c=colors[categories], alpha=t, s=sizes[categories])

    dump_label = plt.text(x=0.90, y=0.97, ha='center', va='center', transform=ax.transAxes,
